defmodule Gameboy.Timer do
  ...
  Record.defrecordp(:timer, tima: 0, tma: 0, tac: 0, counter: 0, overflow: false)
  ...
  #   {%{timer | counter: 0}, req}
  # end
  def set_div_cycle(t) do
    {t, req} = cycle(t)
    # When writing to div, div is set to 0.
    # If the counter bit of div is 1, then its change to 0 causes a fallen edge, so tima is incremented
    t = if check_counter_bit(timer(t, :counter), timer(t, :tac)) do
      new_tima = timer(t, :tima) + 1
      if new_tima > 0xff do
        timer(t, tima: 0, overflow: true)
      else
        timer(t, tima: new_tima)
      end
    else
      t
    end
    {timer(t, counter: 0), req}
  end
  ...
  #   end
  # end
  def set_tima_cycle(timer(overflow: overflow) = t, value) do
    {t, req} = cycle(t)
    if overflow do
      # In a cycle following the overflow, write to tima is ignored and overwritten by vaue in tma
      {t, req}
      ...
      # If write to tima occurs in a cycle of overflow, loading from tma in the following cycle is canceled
      # That is why overflow is set to false here
      {timer(t, tima: value, overflow: false), req}
    end
  end
  ...
  #   end
  # end
  def set_tma_cycle(timer(overflow: overflow) = t, value) do
    {t, req} = cycle(t)
    if overflow do
      # If tma is written in a cycle following an overflow, the new vaue of tma is loaded to tima
      {timer(t, tma: value, tima: value), req}
    else
      {timer(t, tma: value), req}
    end
  end
  ...
  #   end
  # end
  def set_tac_cycle(t, value) do
    {t, req} = cycle(t)
    timer(tac: tac, counter: counter) = t
    old_bit = elem(@timer_enable, tac) and check_counter_bit(counter, tac)
    new_tac = elem(@tac_value, value)
    new_bit = elem(@timer_enable, new_tac) and check_counter_bit(counter, new_tac)
    # If counter bit goes from 1 -> 0, there's a fallen edge, so increment
    # This happens even when counter bit to monitor is changed
    # Same thing occurs when the timer is disabled
    if old_bit and !new_bit do
      new_tima = timer(t, :tima) + 1
      if new_tima > 0xff do
        {timer(t, tima: 0, overflow: true, tac: new_tac), req}
      else
        {timer(t, tima: new_tima, tac: new_tac), req}
      end
    else
      {timer(t, tac: new_tac), req}
    end
  end
  ...
  #   end
  # end
  def cycle(timer(overflow: true, tma: tma, counter: counter) = t) do
    # Request interrupt
    # Timer interrupt and load from tma is delayed 1 cycle from an actual overflow
    {timer(t, counter: (counter + 1) &&& 0xffff, tima: tma, overflow: false), Interrupts.timer()}
  end
  def cycle(timer(overflow: false, counter: counter, tima: tima, tac: tac) = t) do
    new_counter = (counter + 1) &&& 0xffff
    if elem(@timer_enable, tac) and check_counter_bit(counter, tac) do
      # Detected a fallen edge on a counter bit = increment, ex. 0x0111 -> 0x1000
      if !check_counter_bit(new_counter, tac) do
        new_tima = tima + 1
        if new_tima > 0xff do # tima overflow
          {timer(t, counter: new_counter, tima: 0, overflow: true), 0}
        else
          {timer(t, counter: new_counter, tima: new_tima), 0}
        end
      else
        {timer(t, counter: new_counter), 0}
      end
    else
      {timer(t, counter: new_counter), 0}
    end
